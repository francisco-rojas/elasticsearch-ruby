...
module LegacyElasticsearch
  module Test
    class UtilsTest < ::Test::Unit::TestCase
      include LegacyElasticsearch::API::Utils
      ...
      context "Utils" do
        ...
        context "__bulkify" do
          ...
          should "serialize array of hashes" do
            result = LegacyElasticsearch::API::Utils.__bulkify [
          ...
          should "serialize arrays of strings" do
            result = LegacyElasticsearch::API::Utils.__bulkify ['{"foo":"bar"}','{"moo":"bam"}']
            assert_equal <<-PAYLOAD.gsub(/^\s+/, ''), result
              {"foo":"bar"}
              {"moo":"bam"}
            PAYLOAD
          ...
          should "serialize arrays of header/data pairs" do
            result = LegacyElasticsearch::API::Utils.__bulkify [{:foo => "bar"},{:moo => "bam"},{:foo => "baz"}]
            assert_equal <<-PAYLOAD.gsub(/^\s+/, ''), result
              {"foo":"bar"}
              {"moo":"bam"}
              {"foo":"baz"}
          ...
          should "not modify the original payload with the data option" do
            ...
            result = LegacyElasticsearch::API::Utils.__bulkify original
        ...
        context "__validate_and_extract_params" do
          teardown do
            LegacyElasticsearch::API.settings.clear
          end
