...
class LegacyElasticsearch::Transport::Transport::HTTP::FaradayTest < Test::Unit::TestCase
  include LegacyElasticsearch::Transport::Transport::HTTP
  ...
  context "Faraday transport" do
    ...
    should "pass the selector_class options to collection" do
      @transport = Faraday.new :hosts => [ { :host => 'foobar', :port => 1234 } ],
                               :options => { :selector_class => LegacyElasticsearch::Transport::Transport::Connections::Selector::Random }
      assert_instance_of LegacyElasticsearch::Transport::Transport::Connections::Selector::Random,
                         @transport.connections.selector
    end

    should "pass the selector option to collection" do
      @transport = Faraday.new :hosts => [ { :host => 'foobar', :port => 1234 } ],
                               :options => { :selector => LegacyElasticsearch::Transport::Transport::Connections::Selector::Random.new }
      assert_instance_of LegacyElasticsearch::Transport::Transport::Connections::Selector::Random,
                         @transport.connections.selector
    end
