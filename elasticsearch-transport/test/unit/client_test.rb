...
class LegacyElasticsearch::Transport::ClientTest < Test::Unit::TestCase
  ...
  context "Client" do
    setup do
      LegacyElasticsearch::Transport::Client::DEFAULT_TRANSPORT_CLASS.any_instance.stubs(:__build_connections)
      @client = LegacyElasticsearch::Transport::Client.new
    end

    should "be aliased as LegacyElasticsearch::Client" do
      assert_nothing_raised do
        assert_instance_of(LegacyElasticsearch::Transport::Client, LegacyElasticsearch::Client.new)
      end
    end

    should "have default transport" do
      assert_instance_of LegacyElasticsearch::Transport::Client::DEFAULT_TRANSPORT_CLASS, @client.transport
    end

    should "instantiate custom transport class" do
      client = LegacyElasticsearch::Transport::Client.new :transport_class => DummyTransport
      assert_instance_of DummyTransport, client.transport
    end

    should "take custom transport instance" do
      client = LegacyElasticsearch::Transport::Client.new :transport => DummyTransport.new
      assert_instance_of DummyTransport, client.transport
    end
    ...
    should "send GET request as POST with the send_get_body_as option" do
      ...
      client = LegacyElasticsearch::Transport::Client.new :transport => transport, :send_get_body_as => 'POST'
    ...
    should "have default logger for transport" do
      client = LegacyElasticsearch::Transport::Client.new :log => true
      assert_respond_to client.transport.logger, :info
    end

    should "have default tracer for transport" do
      client = LegacyElasticsearch::Transport::Client.new :trace => true
      assert_respond_to client.transport.tracer, :info
    end

    should "initialize the default transport class" do
      LegacyElasticsearch::Transport::Client::DEFAULT_TRANSPORT_CLASS.any_instance.
    ...
    should "pass options to the transport" do
      client = LegacyElasticsearch::Transport::Client.new :transport_options => { :foo => 'bar' }
      assert_equal 'bar', client.transport.options[:transport_options][:foo]
    end

    should "merge request_timeout to the transport options" do
      client = LegacyElasticsearch::Transport::Client.new :request_timeout => 120
    ...
    context "when passed hosts" do
      should "have localhost by default" do
        c = LegacyElasticsearch::Transport::Client.new
        assert_equal 'localhost', c.transport.hosts.first[:host]
      end

      should "take :hosts, :host, :url or :urls" do
        c1 = LegacyElasticsearch::Transport::Client.new :hosts => ['foobar']
        c2 = LegacyElasticsearch::Transport::Client.new :host  => 'foobar'
        c3 = LegacyElasticsearch::Transport::Client.new :url   => 'foobar'
        c4 = LegacyElasticsearch::Transport::Client.new :urls  => 'foo,bar'
    ...
    context "detecting adapter for Faraday" do
      setup do
        LegacyElasticsearch::Transport::Client::DEFAULT_TRANSPORT_CLASS.any_instance.unstub(:__build_connections)
      ...
      should "use the adapter from arguments" do
        c = LegacyElasticsearch::Transport::Client.new :adapter => :typhoeus
        handlers = c.transport.connections.all.first.connection.builder.handlers
    ...
    context "configuring Faraday" do
      ...
      should "apply faraday adapter" do
        c = LegacyElasticsearch::Transport::Client.new do |faraday|
    ...
    context "when passed options" do
      ...
      should "transfer selected host parts from URL into the 'http' options" do
        c = LegacyElasticsearch::Transport::Client.new :url => 'https://U:P@node1:8080'
