...
class LegacyElasticsearch::Transport::ClientProfilingTest < LegacyElasticsearch::Test::ProfilingTest
  startup do
    LegacyElasticsearch::Extensions::Test::Cluster.start if ENV['SERVER'] and not LegacyElasticsearch::Extensions::Test::Cluster.running?
  end

  context "Elasticsearch client benchmark" do
    setup do
      @port = (ENV['TEST_CLUSTER_PORT'] || 9250).to_i
      client = LegacyElasticsearch::Client.new host: "localhost:#{@port}", adapter: ::Faraday.default_adapter
    ...
    teardown do
      client = LegacyElasticsearch::Client.new host: "localhost:#{@port}"
    ...
    context "with a single-node cluster and the default adapter" do
      setup do
        @client = LegacyElasticsearch::Client.new hosts: "localhost:#{@port}", adapter: ::Faraday.default_adapter
      end
    ...
    context "with a two-node cluster and the default adapter" do
      setup do
        @client = LegacyElasticsearch::Client.new hosts: ["localhost:#{@port}", "localhost:#{@port+1}"], adapter: ::Faraday.default_adapter
      end
    ...
    context "with a single-node cluster and the Curb client" do
      setup do
        ...
        @client = LegacyElasticsearch::Client.new host: "localhost:#{@port}",
                                            transport_class: LegacyElasticsearch::Transport::Transport::HTTP::Curb
      end
    ...
    context "with a single-node cluster and the Patron adapter" do
      setup do
        require 'patron'
        @client = LegacyElasticsearch::Client.new host: "localhost:#{@port}", adapter: :patron
      end
