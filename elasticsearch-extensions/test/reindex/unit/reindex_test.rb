...
class LegacyElasticsearch::Extensions::ReindexTest < Test::Unit::TestCase
  context "The Reindex extension module" do
    ...
    should "require options" do
      assert_raise ArgumentError do
        LegacyElasticsearch::Extensions::Reindex.new
      end
    end

    should "allow to initialize the class" do
      assert_instance_of LegacyElasticsearch::Extensions::Reindex::Reindex,
                         LegacyElasticsearch::Extensions::Reindex.new(DEFAULT_OPTIONS)
    end

    should "add the reindex to the API and client" do
      assert_includes LegacyElasticsearch::API::Actions.public_instance_methods.sort, :reindex
      assert_respond_to LegacyElasticsearch::Client.new, :reindex
    end

    should "pass the client when used in API mode" do
      client = LegacyElasticsearch::Client.new

      LegacyElasticsearch::Extensions::Reindex::Reindex
        .expects(:new)
    ...
    context "when performing the operation" do
      ...
      should "scroll through the index and save batches in bulk" do
        client  = mock()
        subject = LegacyElasticsearch::Extensions::Reindex.new source: { index: 'foo', client: client },
                                                         target: { index: 'bar' }
      ...
      should "transform the documents with a lambda" do
        client  = mock()
        subject = LegacyElasticsearch::Extensions::Reindex.new \
