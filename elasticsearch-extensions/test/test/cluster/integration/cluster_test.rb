...
class LegacyElasticsearch::Extensions::TestClusterIntegrationTest < Test::Unit::TestCase
  context "The Test::Cluster" do
    ...
    @builds.each do |build|
      should "start and stop #{build.to_s}" do
        ...
        LegacyElasticsearch::Extensions::Test::Cluster.start command: PATH_TO_BUILDS.join(build.join('bin/elasticsearch')).to_s
        LegacyElasticsearch::Extensions::Test::Cluster.stop command: PATH_TO_BUILDS.join(build.join('bin/elasticsearch')).to_s
      end
    end
  end
end
